...
def test_all_departure_stations():
    ...
    routes = [TrainRoute(1, "11022", "AAA", "NGP", "CSMT", "08:00", "23:00", 10, "mon")]
    stations = all_departure_stations(routes)
    assert stations == ["NGP"]
...
def test_find_available_routes():
    ...
    train_2 = TrainRoute(
        1, "11020", "AAA", "NGP", "CSMT", "08:00", "20:00", 30, "mon/fri"
    )
    train_3 = TrainRoute(
        1, "11050", "AAA", "KYN", "CSMT", "08:00", "20:00", 30, "mon/fri"
    )

    routes = [train_1]
    available_seats = {train_1.id: 1}
    available_routes = find_available_routes(
        routes, "NGP", "CSMT", "mon", available_seats
    )
    ...
    available_seats = {train_1.id: 1}
    available_routes = find_available_routes(
        routes, "NGP", "CSMT", "tue", available_seats
    )
    assert available_routes == []
