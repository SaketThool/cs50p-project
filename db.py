...
def db_setup():
    ...
    cur = con.cursor() # cur is the cursor which help to read and write in the data base   
...
def get_all_routes(con): 
    ...
    rows = cur.execute("SELECT * FROM routes").fetchall() # In this we get all the rows and columns from the route table.'*' means all columns.  
...
def save_booking(con, booking): # This function help to save the booking details of the user which is been input and give back PNR
    cur = con.cursor()
    ...
    return cur.lastrowid  # The primary key of the table is PNR which is automatically created
...
def get_booking(con, pnr): # This function help to get the booking information for the chosen PNR that has been stored in booking table
    cur = con.cursor()
    ...
    if row: # If we find a booking with given PNR it will continue 
        ...
        booking = Booking(
           route, 
           date.fromisoformat(row[2]), # row[2] is a string(yyyy-mm-dd) convert into the python date object
...
def setup_station_table(cur):  # This function use to store the station name and station code in the table
    ...
    if no_of_stations == 0:
        try:
            with open("train_stations.csv", "r") as csvfile:  # This line opens the file object
...
def setup_routes_table(cur):   # This function use to store the train route details in the table
...
def setup_booking_table(cur):  # This function use to store the booking detail of the user in the table
